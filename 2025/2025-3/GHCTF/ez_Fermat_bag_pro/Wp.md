首先是把n分解得到p，这一步和ImaginaryCTF二月赛的bivariate一样

思路是把p,q带入 $f(x,y)$ 得到 $f(p,q) = p^{31} -p^{30}q + ...+2q -2$ 。这时候把 $f(p,q)$ 模掉 $p-1$ ，此时得到的 $f(p,q)$ 只剩下包含q的多项式，再把n带入
...
# print(a)
y = n
a = y^31 + y^30 - y^29 - 6*y^26 + 16*y^24 - 6*y^23 - y^22 - 6*y^21 - 10*y^20 + 2*y^19 - 6*y^18 - 2*y^17 + 13*y^16 - y^15 - 2*y^14 - 8*y^13 - 5*y^12 + 17*y^11 + 4*y^10 - 3*y^9 - 2*y^8 - 17*y^7 - 4*y^6 - 9*y^5 - 7*y^4 + 11*y^3 + 6*y^2 - 4*y - 4

p = gmpy2.gcd(pow(2,a,n)-w,n)
```

得到p之后就是鸡块给NSSCTF出的easy_mod那一系列题
...
而flag已知头尾，中间由0~9的数字构成


$$
c \equiv tmp + m_{unknown} \mod p
$$


这时候再把 $m_{unknwon}$ 看作 $m_{unknwon} = m_0256^{0} + m_1256^1 + ...+m_n256^n$
...
for line in M.BKZ():
    flag = ''
    if abs(line[-2]) == 5 and all(abs(i) < 10 for i in line[:-2]):
        for i in line[:-2][::-1]:
            flag += str(abs(i))
    print("NSSCTF{" + flag + "}")
    break
